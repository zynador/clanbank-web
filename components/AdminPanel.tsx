...
export default function AdminPanel() {
  ...
  async function handleResetPassword() {
    if (!selectedUserId || !newPassword || newPassword.length < 6) {
      setFeedback({
        type: 'error',
        text: lang === 'de' ? 'Mitglied wählen + Passwort mind. 6 Zeichen.' : 'Select member + password min. 6 chars.',
      })
      return
    }
    setResetting(true)
    try {
      ...
      if (result.success) {
        setFeedback({
          type: 'success',
          text: lang === 'de' ? '✅ Passwort gesetzt. Bitte an Spieler weitergeben.' : '✅ Password set. Share it with the player.',
        })
        setNewPassword('')
        setSelectedUserId('')
      } else {
  ...
  async function handleToggleBank() {
    ...
    const { error } = await supabase.from('profiles').update({ is_bank: newValue }).eq('id', bankTargetId)
    if (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Einladungscode — für Demo-User ausgeblendet */}
      {isDemo ? (
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-5">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <button
              onClick={() => { navigator.clipboard.writeText('MAFIA2026'); setFeedback({ type: 'success', text: t.copied[lang] }) }}
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {inviteCode && (
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => { navigator.clipboard.writeText(inviteCode); setFeedback({ type: 'success', text: t.copied[lang] }) }}
      ...
      <div data-tour-id="admin-password-reset" className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-5">
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            {newPassword && (
              <button
                onClick={() => { navigator.clipboard.writeText(newPassword); setFeedback({ type: 'success', text: t.pw_copy[lang] + ' ✓' }) }}
                className="px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-400 text-sm hover:bg-zinc-700 transition-colors"
              >
                {'📋'}
              </button>
      ...
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-5">
        ...
        <div className="space-y-3">
          <select
            value={bankTargetId}
            onChange={e => setBankTargetId(e.target.value)}
            className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-200 text-sm focus:outline-none focus:border-zinc-500"
          >
      ...
      {/* Bankstand-Import */}
      <div data-tour-id="admin-bank-import" className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-5">
