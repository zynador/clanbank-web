...
export default function FCUEventTab({ lang }: Props) {
  ...
  async function handleCreate() {
    ...
    if (error || !(data as { success: boolean })?.success) {
      setFeedback((data as { message: string })?.message || (lang === 'de' ? 'Fehler beim Anlegen.' : 'Error creating event.'))
      return
    }
    setShowForm(false)
    setEventName('')
    setEventDate('')
    await loadEvents()
    setActiveEventId((data as { fcu_event_id: string }).fcu_event_id)
    setView('upload')
  }

  function openUpload(eventId: string) { setActiveEventId(eventId); setView('upload') }
  function openResults(eventId: string) { setActiveEventId(eventId); setView('results') }
  function backToList() { setView('list'); setActiveEventId(null); loadEvents() }
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {loading ? (
        ...
        <div className="space-y-2" data-tour-id="fcu-list">
          {events.map(ev => (
