...
type BacklogMember = { user_id: string; ingame_name: string; weeks_behind: number; missing_resources: ResourceType[]; is_starter?: boolean }
type MyStatus = { weeks_behind: number; missing_resources: ResourceType[] }
type MemberDetail = { user_id: string; ingame_name: string; weeks_behind: number; total_all_time: number; per_resource: Record<ResourceType, number>; is_starter?: boolean }
...
const RESOURCE_LABELS: Record<ResourceType, { emoji: string; label: string }> = {
  cash: { emoji: '💵', label: 'Cash' }, arms: { emoji: '🔫', label: 'Arms' },
  cargo: { emoji: '📦', label: 'Cargo' }, metal: { emoji: '⚙️', label: 'Metal' },
  diamond: { emoji: '💎', label: 'Diamond' },
}
const RESOURCE_COLORS: Record<ResourceType, string> = {
  cash: 'bg-yellow-100 text-yellow-800', arms: 'bg-red-100 text-red-800',
  cargo: 'bg-cyan-100 text-cyan-800', metal: 'bg-gray-200 text-gray-700',
...
export default function HomeTab({ lang, onNavigate }: Props) {
  ...
  const t = {
    ...
    statusOk: lang === 'de' ? 'TGM Consigliere: Du bist auf dem Laufenden' : 'TGM Consigliere: You are up to date',
    statusBehind: lang === 'de' ? 'TGM Consigliere: Du bist im Rückstand!' : 'TGM Consigliere: You are behind!',
  ...
  async function loadMyStatus() {
    ...
    if ((profile as unknown as Record<string, unknown>).is_raidleiter) {
      setMyStatus({ weeks_behind: 0, missing_resources: [] }); return
    }
    const now = new Date()
    ...
    for (let w = currentKw - 1; w >= currentKw - 4; w--) {
      ...
      if (!hasAny) weeksBehind++; else break
    }
  ...
  async function loadBacklog() {
    ...
    const activeProfiles = (allProfiles ?? []).filter(p => {
      ...
      profileNames.add((p.ingame_name ?? '').toLowerCase()); return true
    })
    const { data: starterRows } = await supabase
      .from('starter_members').select('id, ingame_name')
      .eq('clan_id', profile.clan_id).is('claimed_by', null).is('left_clan_at', null)
    ...
    for (const p of activeProfiles) {
      ...
      result.push({ user_id: p.id, ingame_name: p.ingame_name, weeks_behind: weeksBehind, missing_resources: missing })
    }
    ...
    for (const s of activeStarters) {
      ...
      if (missing.length > 0) startersBehind.push(entry); else startersPaid.push(entry)
    }
  ...
  async function loadDetailForStarter(member: BacklogMember) {
    ...
    const { data: histData } = await supabase
      ...
      .eq('ingame_name', member.ingame_name).eq('clan_id', profile?.clan_id ?? '')
  ...
  async function loadDetailForMember(member: BacklogMember) {
    ...
    const { data: histData } = await supabase
      .from('historical_deposits').select('resource_type, amount')
      .eq('ingame_name', member.ingame_name).eq('clan_id', profile?.clan_id ?? '').eq('transferred', false)
    for (const d of histData ?? []) {
      const rt = (d.resource_type as string).toLowerCase() as ResourceType
      if (perResource[rt] !== undefined) { perResource[rt] += d.amount; totalAllTime += d.amount }
    }
  ...
  async function loadDetail(member: BacklogMember) {
    ...
    setSelectedId(member.user_id); setDetailLoading(true); setDetail(null)
    const { perResource, totalAllTime } = member.is_starter
      ? await loadDetailForStarter(member)
      : await loadDetailForMember(member)
    setDetail({ user_id: member.user_id, ingame_name: member.ingame_name, weeks_behind: member.weeks_behind, total_all_time: totalAllTime, per_resource: perResource, is_starter: member.is_starter })
  ...
  async function loadBankRanking() {
    ...
    try {
      const { data: profileData } = await supabase
        .from('profiles').select('id, is_bank, is_raidleiter, is_test').eq('clan_id', profile.clan_id)
      const bankIds = new Set<string>(); const testIds = new Set<string>(); const raidleiterIds = new Set<string>()
      ...
      const sorted = (data as unknown as Record<string, unknown>[])
        ...
        .slice(0, 5)
        .map(r => ({ user_id: (r.user_id ?? r.ingame_name) as string, ingame_name: (r.ingame_name || r.username || '?') as string, total_amount: Number(r.total_deposit), is_raidleiter: raidleiterIds.has(r.user_id as string) }))
  ...
  async function loadFcuRanking() {
    ...
    if (!data) return
    setFcuRanking((data as unknown as Record<string, unknown>[]).slice(0, 5).map(d => ({ ingame_name: d.ingame_name as string, total_points: Number(d.total_points) })))
  }
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {/* Wand der Schande */}
      <div data-tour-id="home-backlog" className={'rounded-xl p-3 space-y-3 border ' + (hasAnyBehind ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200')}>
        <div className="flex items-center justify-between">
          <p className="text-sm font-medium text-gray-800">{'⚠️ ' + t.backlogTitle}</p>
          {totalMembers > 0 && <span className="text-xs text-gray-500">{paidCount + '/' + totalMembers + ' ' + t.paid}</span>}
        </div>
        {backlog.length === 0 ? (
      ...
      {starters.length > 0 && (
        <div className="rounded-xl p-3 space-y-2 border bg-blue-50 border-blue-200">
          ...
          <div className="grid gap-1.5" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))' }}>
            {starters.map(member => (
              <button key={member.user_id} onClick={() => loadDetail(member)}
                className={'text-left bg-white rounded-lg px-2.5 py-2 border transition-colors ' + (selectedId === member.user_id ? 'border-blue-400 ring-1 ring-blue-300' : 'border-blue-100 hover:border-blue-300')}>
                <p className="text-xs font-medium text-gray-800 truncate mb-1">{member.ingame_name}</p>
            ...
            <>
              <div className="grid grid-cols-3 gap-1 items-end mb-2">
                {[1, 0, 2].map((i: number) => {
                  ...
                  const isGold = i === 0; const isSilver = i === 1
                  return (
                    <div key={i} className={'rounded-lg p-1.5 text-center border ' + (isGold ? 'border-yellow-200 bg-yellow-50' : isSilver ? 'border-gray-200 bg-gray-50' : 'border-orange-100 bg-orange-50')}>
                      <p className="text-xs mb-0.5" style={{ color: isGold ? '#b7950b' : isSilver ? '#7f8c8d' : '#a04000' }}>{(i + 1) + '.'}</p>
                      <div className={isGold ? 'text-xl mb-1' : 'text-base mb-1'}>{isGold ? '🏆' : isSilver ? '🥈' : '🥉'}</div>
                      <p className={'text-xs font-medium truncate ' + (isGold ? 'text-yellow-700' : 'text-gray-700')}>{entry.ingame_name}{entry.is_raidleiter ? ' ⚔️' : ''}</p>
              ...
              {bankRanking.slice(3, 5).map((entry, idx) => (
                <div key={entry.user_id} className="flex items-center gap-1.5 py-1 border-t border-gray-50">
                  ...
                  <span className="flex-1 text-xs text-gray-700 truncate">{entry.ingame_name}{entry.is_raidleiter ? ' ⚔️' : ''}</span>
                ...
                {[1, 0, 2].map((i: number) => {
                  ...
                  return (
                    <div key={i} className={'rounded-lg p-1.5 text-center border ' + (isGold ? 'border-yellow-200 bg-yellow-50' : isSilver ? 'border-gray-200 bg-gray-50' : 'border-orange-100 bg-orange-50')}>
                      <p className="text-xs mb-0.5" style={{ color: isGold ? '#b7950b' : isSilver ? '#7f8c8d' : '#a04000' }}>{(i + 1) + '.'}</p>
                      <div className={isGold ? 'text-xl mb-1' : 'text-base mb-1'}>{isGold ? '🏆' : isSilver ? '🥈' : '🥉'}</div>
                      <p className={'text-xs font-medium truncate ' + (isGold ? 'text-yellow-700' : 'text-gray-700')}>{entry.ingame_name}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className={'flex items-center justify-between px-4 pt-4 pb-2 ' + (detail?.is_starter ? 'border-b border-blue-100' : 'border-b border-red-100')}>
              ...
              <button onClick={closeDetail} className="text-gray-400 hover:text-gray-600 text-lg leading-none ml-2 flex-shrink-0" aria-label="Modal schließen">
                {'✕'}
              </button>
            </div>
            <div className="px-4 pb-4 space-y-3 pt-3">
              {detailLoading ? (
                ...
                <>
                  {detail.is_starter && (
                    <p className="text-xs text-blue-600 bg-blue-50 rounded-lg px-3 py-2">{t.historicalData}</p>
                  )}
                  <div className="grid grid-cols-2 gap-2">
